...
* 偏向锁：Mark Word存储了拥有该偏向锁的线程ID，线程进入临界区时只需要探测是否存储的是指向该线程的偏向锁，如果是，直接进入临界区。否则看当前的锁标志是否为1(即偏向锁)，如果是，则CAS将偏向锁指向该线程（当一个线程发生锁的重入时），如果是无锁状态，则CAS竞争锁。当发生竞争时，偏向锁尝试进行撤销，如果占用偏向锁的线程还活着，锁会升级（线程争用一把锁）。
